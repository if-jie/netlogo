globals [number-of-robots
         goal goal-found
         learning-rate exploration-rate discount-rate;学习率，探索，折扣率
         q-table;Q值表
         times; 一轮的所需时间
         random-times exploit-times random-unexplored exploit-unexplored
         testing random-policy exploit ; using random or learned behavior
         filename] ;;  record the times
turtles-own [start-state action end-state
             dist-to-goal  ;;乌龟到目标点的距离（用于计算奖励）
             turned-towards-obstacle ;;计算奖励时使用
...
to setup
  clear-all
  set test-mode false  ;测试模式
  set number-of-robots 6  ;设置数量
  set learning-rate 1
  set exploration-rate 1 ;探索率
  set discount-rate 0.5 ;; ;折扣率平衡奖励
  set times (list)
  set random-times (list)
  set exploit-times (list)
  set random-unexplored (list)
  set exploit-unexplored (list) ;未探索方格数的列表
  set goal patch 0 0 ;; 仅用于确保开始时存在目标点
  set filename "test" ; 设置输出文件名
  set testing false
  set random-policy false ; 设置随机策略为假
  set exploit false ; 设置优化策略为假
  set learn true ; 设置学习为真


  ; 初始化 Q-表，全部为 0；应通过 STATE（项目 ACTION q-table）访问 Q-表，其中 0 <= STATE < 324，0 <= ACTION < 4
...
to start-round
  ...
  create-turtles number-of-robots [set start-state (n-values 4 [black])
                    ...
                    set heading 0] ;initialize state to a length 4 list of the color
...
to create-goal
  ...
  while [ done = 0 ] [
    ...
    let p patch x y
    if ([pcolor] of p != brown) and (p != patch 0 0)  [
...
;;this will generate brown squares at a point on the map
;;each brown patch will be treated like a wall, so they will act as obstacles
...
to spawn-obstacle
  let obstacle-size random 6 ;;how many squares around the center point will be turned brown
  ...
  set pcolor brown ;;the center patch will make itself brown
      ...
      repeat 3 [
        if x-difference != 0 or y-difference != 0 [ ;;making sure its not checking itself
          ...
          and [pcolor] of patch (pxcor + x-difference) (pycor + y-difference) != brown [
            ;;if the area is clear, turn the patch brown
...
to-report dfs
  ...
  while [not empty? stack] [
    ...
    ask neighbor-set [ ;; ask each neighbor patch to do the following:
      ...
      if (not member? self visited) and (pcolor != brown) [
